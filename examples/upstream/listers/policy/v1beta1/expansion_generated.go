...
// EvictionClusterListerExpansion allows custom methods to be added to
// EvictionLister.
type EvictionClusterListerExpansion interface {
	// Cluster returns a lister that can list and get Eviction in one workspace.
	Cluster(clusterName logicalcluster.Name) EvictionLister
}
...
type PodDisruptionBudgetClusterListerExpansion interface {
	// Cluster returns a lister that can list and get PodDisruptionBudget in one workspace.
