...
type StatefulSetClusterInformer interface {
	...
	Lister() v1.StatefulSetClusterLister
	Cluster(logicalcluster.Name) upstreamappsv1informers.StatefulSetInformer
}
...
func NewStatefulSetClusterInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
func (f *statefulSetClusterInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *statefulSetClusterInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&appsv1.StatefulSet{}, f.defaultInformer)
}

func (f *statefulSetClusterInformer) Lister() v1.StatefulSetClusterLister {
	return v1.NewStatefulSetClusterLister(f.Informer().GetIndexer())
}
