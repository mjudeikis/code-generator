...
type DeviceClassClusterInformer interface {
	...
	Lister() v1alpha3.DeviceClassClusterLister
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredDeviceClassInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *deviceClassClusterInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&resourcev1alpha3.DeviceClass{}, f.defaultInformer)
}

func (f *deviceClassClusterInformer) Lister() v1alpha3.DeviceClassClusterLister {
	return v1alpha3.NewDeviceClassClusterLister(f.Informer().GetIndexer())
}
